...
# Fonction pour fusionner les données réelles et les prédictions
def get_data_from_s3(symbol):
    # Charger les données réelles depuis S3
    obj = s3.get_object(Bucket=BUCKET, Key=f'DataHistory/{symbol}.csv')
    df_real = pd.read_csv(obj['Body'])

    # Charger les prédictions depuis S3
    obj = s3.get_object(Bucket=BUCKET, Key=f'StockPredictions/{symbol}.csv')
    df_pred = pd.read_csv(obj['Body'])

    # Prendre uniquement les colonnes pertinentes
    df_real = df_real[['Date', 'Close']]
    df_pred = df_pred[['Predicted_Close']]

    # Convertir la colonne "Date" en format datetime
    df_real['Date'] = pd.to_datetime(df_real['Date'])

    # Récupérer la dernière date des données réelles
    last_real_date = df_real['Date'].max()

    # Générer les dates manquantes pour les 30 jours suivants (Pour ne pas compter les week-ends)
    future_dates = []
    ...
    # Prendre les 30 dernières lignes du CSV de prédiction
    df_pred = df_pred.tail(30).copy()

    # Ajouter les dates futures générées aux 30 dernières lignes du df_pred
    df_pred['Date'] = future_dates

    # Renommer la colonne "Predicted_Close" en "Close"
    df_pred = df_pred.rename(columns={'Predicted_Close': 'Close'})

    # Combiner les données réelles et les prédictions
    df_combined = pd.concat([df_real, df_pred])

    return df_combined

# Modèle de stratégie d'évolution profonde
    ...
    def buy(self):
        ...
        for t in range(0, len(self.trend) - 1, self.skip):
            ...
            if action == 1 and initial_money >= self.trend[t]:
                ...
                states_sell[t] = 'Day %d, Sell 1 unit at Price %.2f, Investment %.2f %%, Total Balance %.2f' % (t, self.trend[t], invest, initial_money)
            state = next_state
        invest = ((initial_money - starting_money) / starting_money) * 100
        total_gains = initial_money - starting_money
        return states_buy, states_sell, total_gains, invest

# Fonction principale pour exécuter le trading sur les données d'un symbole
def trade_stock(symbol, period, init, skip):
    # Charger les données depuis S3 et générer le DataFrame final
    df = get_data_from_s3(symbol)

    close = df['Close'].values.tolist()
    date_ori = pd.to_datetime(df['Date']).dt.strftime('%Y-%m-%d').tolist()
    window_size = 30
    initial_money = init
